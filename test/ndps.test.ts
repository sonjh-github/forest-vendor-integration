...
const ndpsRegisterFixture = {
  vendor: "NDPS", reportedByDeviceId: "NMS-1", observedAt,
  devices: [{ vendorDeviceId: "NMS-1", deviceType: "TVWS_NMS" }, { vendorDeviceId: "BASE-1", deviceType: "TVWS_BASE" }, { vendorDeviceId: "CPE-1", deviceType: "TVWS_CPE" }],
};
...
// 정상 시나리오: 장비 등록과 상태 변형을 검증한다.
test("NDPS: NMS·Base·CPE 장비 등록 (임시 값)", () => {
  assert.doesNotThrow(() => assertNdpsRegisterRequest(ndpsRegisterFixture));
});
