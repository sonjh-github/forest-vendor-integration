...
jininfraRoutes.post("/register", async (c) => {
  try {
...
jininfraRoutes.post("/invoke", async (c) => {
  try {
    ...
    const body = await c.req.json();
    assertJininfraInvokeRequest(body);
    const data = await invokeJininfra(body, mode, c.req.header("Idempotency-Key"));
